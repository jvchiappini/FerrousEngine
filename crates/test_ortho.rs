fn main() {
    let wf = 1920.0;
    let hf = 1080.0;
    let OPENGL_TO_WGPU_MATRIX = glam::Mat4::from_cols(
    ...
    let ortho = glam::Mat4::orthographic_rh(0.0, 1920.0, 1080.0, 0.0, -1.0, 1.0);
    ...
    let world_pos4 = shader_model * pos;

    println!("Output Vertex Top-Left (-250, -150) -> {:?}", world_pos4);

    let pos2 = glam::Vec4::new(250.0, 150.0, 0.0, 1.0); // Bottom-right vertex
    let world_pos4_2 = shader_model * pos2;
    println!("Output Vertex Bottom-Right (250, 150) -> {:?}", world_pos4_2);
}
