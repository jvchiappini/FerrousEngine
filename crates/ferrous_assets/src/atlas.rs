...
// path commands not needed directly here
/// UV and size metrics for a glyph inside the atlas texture.
#[derive(Debug, Clone)]
...
impl FontAtlas {
    pub fn new<I: IntoIterator<Item = char>>(
        ...
        if count == 0 {
            return Err(anyhow::anyhow!("no characters provided for atlas"));
        }
        ...
        for &c in &char_list {
            ...
            let u0 = x as f32 / tex_size as f32;
            let v0 = y as f32 / tex_size as f32;
            let u1 = (x + glyph_size) as f32 / tex_size as f32;
            let v1 = (y + glyph_size) as f32 / tex_size as f32;
            metrics.insert(c, GlyphMetrics { uv: [u0, v0, u1, v1], size: [glyph_size as f32, glyph_size as f32] });
            x += glyph_size;
        ...
        queue.write_texture(
            ...
            wgpu::ImageDataLayout {
                offset: 0,
                bytes_per_row: Some((tex_size * 4).try_into().unwrap()),
                rows_per_image: None,
            },
